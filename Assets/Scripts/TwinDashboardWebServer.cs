...
<script>
const q=id=>document.getElementById(id), hist=[];let socket,snap={};
...
function apply(){send({type:'setpoints',conveyorSpeedMps:+q('conv').value,pumpFlowLitersPerMinute:+q('pump').value,infeedMotorSpeedRpm:+q('rpm').value})}
function preset(p){send({type:'preset',preset:p})}function togglePause(){send({type:'pause',paused:!snap.paused})}
for(const id of ['conv','pump','rpm'])q(id).oninput=()=>{q(id+'Out').textContent=id==='conv'?fmt(q(id).value,2)+' m/s':id==='pump'?fmt(q(id).value,0)+' L/min':fmt(q(id).value,1)+' rpm'};
